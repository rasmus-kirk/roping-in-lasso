...
- *Soundness:* $forall prover^* in "ITM", X in.not in L ==> Pr[verifier_"out" = top] <= epsilon(X)$
...
- *Soundness*: $forall prover^* in "PPT", X in.not L ==> Pr[verifier_"out" = top] <= epsilon(X)$
...
- *Zero-Knowledge:* For every PPT verifier $V^*$, there exists a PPT
  ...
  bits^*$, the transcripts produces for $S$ are indistinguishable from
  the interaction between any verifier and an honest prover:
  $ S(x, z) tilde View_(V^*) [P(x) arrow.l.r V^*(z)] $

Where $(tilde)$ denotes indistinguishability. The flavor of Zero-Knowledge
depends on the of the indistinguishability of the transcripts.
...
- *Perfect Zero-Knowledge:* $S(x, z) tilde^P View_(V^*) [P(x) arrow.l.r V^*(z)]$.
- *Statistical Zero-Knowledge:* $S(x, z) tilde^S View_(V^*) [P(x) arrow.l.r V^*(z)]$.
- *Computational Zero-Knowledge:* $S(x, z) tilde^C View_(V^*) [P(x) arrow.l.r V^*(z)]$.
...
#definition(title: "PCS Completeness")[
  For every maximum degree bound $D = poly(lambda) in NN$ and publicly agreed
  upon $d in NN$:
  #prob-game(
    $
      deg(p) <= d <= D, \
...
#definition(title: "Knowledge Soundness")[
  For every maximum degree bound $D = poly(lambda) in NN$, polynomial-size
...
Meaning that $p(X) = q(X)$ with probability at least $frac(style: "horizontal",
d, Fb)$.
