...
Let the sequence of operations be $op_1, ..., op_M$. We maintain a global clock $T$, initialized to $0$.
...
#theorem("Offline Memory Consistency")[
...
We must prove that if $Init union W equiv R union Audit$, the memory has been untampered with. That is, every Read operation retrieved the value from the immediately preceding Write operation to that address.
